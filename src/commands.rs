...
pub fn cmd_status(ctx: &Context) -> Result<()> {
    ...
    for ag in &agents {
        ...
        // C7: show derived agent activity state.
        let activity = crate::squire::agent_activity_status(ctx, ag)
            .map(|s| s.state.as_str())
            .unwrap_or("unknown");
        println!("  {ag:<16} {run:<8} inbox={n} activity={activity}");
    ...
    // C7: project completion derived from durable work state.
    let completion = store::evaluate_project_completion(&ctx.conn)?;
    if completion.complete {
        println!(
            "completion   : COMPLETE (attested by {})",
            completion.attested_by.as_deref().unwrap_or("?")
        );
    } else if completion.eligible {
        println!("completion   : ELIGIBLE (awaiting attestation)");
    } else {
        println!("completion   : INCOMPLETE");
        let blocker_summary: Vec<String> = completion
            .blockers
            .iter()
            .map(|b| b.detail.clone())
            .collect();
        println!("  blockers     : {}", blocker_summary.join("; "));
    }
...
pub fn cmd_work(ctx: &Context, action: &crate::cli::WorkAction) -> Result<()> {
    ...
    match action {
        WorkAction::List {
            ...
            state,
            json,
        } => cmd_work_list(ctx, owner.as_deref(), state.as_deref(), *json),
...
fn cmd_work_add(
    ...
    let assist_policy = AssistPolicy::parse(assist).ok_or_else(|| {
        TrelaneError::msg(format!("unknown assist policy '{assist}'. Known: open, solo"))
    })?;
