...
use crate::models::{
    ...
    ReviewDecision, RunningLock, SplitProposal, StoryEvent, StoryFilter, Task, TaskAssignment,
    TaskReview, TaskRole, TaskState, TaskSubmission, Violation,
};
...
pub fn insert_parked_task(conn: &Connection, task: &ParkedTask) -> Result<()> {
    ...
    // Story ledger (best-effort, R16): record that this agent parked on a
    // wait condition. The wait_type, waiting_on, and the optional wait_re
    // regex go in the typed detail payload per the spec's events.kinds.park.
    // trace_id is None here because store.rs sees only &Connection, not
    // trelane_dir; the spec explicitly allows this (trace_id is nullable).
    let _ = append_story_event(
        conn,
        &StoryEvent::new("park", Some(task.agent.clone()))
            .path(task.wait_path.clone())
            .detail(serde_json::json!({
                "task": task.task,
                "wait_type": task.wait_type,
                "waiting_on": task.waiting_on,
                "wait_re": task.wait_re,
            })),
    );
    Ok(())
}
...
pub fn delete_parked_task(conn: &Connection, task_id: &str) -> Result<()> {
    // Story ledger: capture the parked row's agent BEFORE the DELETE so the
    // unpark event can attribute it. Best-effort; if the lookup fails the
    // event still records the unpark with agent=None (spec allows).
    let agent: Option<String> = conn
        .query_row(
            "SELECT agent FROM parked_tasks WHERE task = ?1",
            params![task_id],
    ...
    let _ = append_story_event(
        conn,
        &StoryEvent::new("unpark", agent).detail(serde_json::json!({
            "task": task_id,
        })),
    );
    Ok(())
...
pub fn insert_claim(conn: &Connection, lease: &Lease) -> Result<bool> {
    ...
    let acquired = n > 0;
    // Story ledger (best-effort, R16): emit claim_acquired on a successful
    // insert, claim_denied when the path was already held. On denial, look
    // up the current holder to enrich the event so a later reader can see
    // who won the contention.
    if acquired {
        let _ = append_story_event(
            conn,
            &StoryEvent::new("claim_acquired", Some(lease.holder.clone()))
                .path(Some(lease.path.clone()))
                .detail(serde_json::json!({
                    "task": lease.task,
                    "delegation_id": lease.delegation_id,
                    "expires_human": lease.expires_human,
                    "grant": lease.grant,
                })),
        );
    } else {
        let current_holder: Option<String> = conn
            .query_row(
                "SELECT holder FROM claims WHERE path = ?1",
                params![lease.path],
                |r| r.get::<_, Option<String>>(0),
            )
            .optional()
        ...
        let _ = append_story_event(
            conn,
            &StoryEvent::new("claim_denied", Some(lease.holder.clone()))
                .path(Some(lease.path.clone()))
                .detail(serde_json::json!({
...
pub fn delete_claim(conn: &Connection, path: &str) -> Result<()> {
    // Story ledger: capture the holder BEFORE the DELETE so the
    // claim_released event can attribute it. Best-effort; the event still
    // records the release (with agent=None) if the lookup fails.
    ...
    let _ = append_story_event(
        conn,
        &StoryEvent::new("claim_released", holder)
...
pub fn release_claims_for_delegation(conn: &Connection, id: &str) -> Result<usize> {
    // Story ledger: capture each freed path's holder BEFORE the DELETE so
    // we can emit one claim_released per path (with agent attribution).
    // The spec explicitly calls out that the bulk path bypasses
    // delete_claim and must be instrumented separately here.
    let paths: Vec<(String, Option<String>)> = {
        let mut stmt = conn.prepare(
            "SELECT path, holder FROM claims WHERE delegation_id = ?1",
        )?;
    ...
    let n = conn.execute("DELETE FROM claims WHERE delegation_id = ?1", params![id])?;
    for (path, holder) in &paths {
        let _ = append_story_event(
            conn,
            &StoryEvent::new("claim_released", holder.clone())
...
// -------------------------------------------------------- story ledger
//
// Append-only causal ledger (see FEATURES/external/trelane-story-ledger-spec.json).
// These are the ONLY functions that touch the `story_events` table, and
// the ONLY writer is `append_story_event`. There is intentionally NO update
// or delete function; the ledger's point is to outlive the ephemeral
// claims/audit_baselines rows. The migration that creates the table
// (db.rs::SCHEMA_V14) declares `seq INTEGER PRIMARY KEY AUTOINCREMENT` to
// document the append-only intent.
//
// R16 best-effort: every caller of `append_story_event` wraps it in
// `let _ = ...` so a ledger insert error can never fail the surrounding
// claim/park/run. `append_story_event` itself returns Result only so that
// callers which WANT to surface errors (e.g. tests) can.

/// Append one event to the ledger. The ONLY write path to `story_events`;
/// never UPDATEs or DELETEs. `seq` is auto-incremented by SQLite; do not
/// set it.
pub fn append_story_event(conn: &Connection, ev: &StoryEvent) -> Result<()> {
    let detail_json = if ev.detail.is_null() {
        "{}".to_string()
    } else {
        serde_json::to_string(&ev.detail)?
    };
    let refs_json = serde_json::to_string(&ev.refs)?;
    conn.execute(
        "INSERT INTO story_events
            (event_id, trace_id, ts_iso, ts_nanos, kind, agent, path,
             hash_before, hash_after, detail_json, refs_json)
...
/// Read events from the ledger in append (seq) order. Read-only. Used by the
/// `trelane story` renderer; all filters optional.
pub fn read_story_events(
    conn: &Connection,
    filter: &StoryFilter,
) -> Result<Vec<StoryEvent>> {
    read_story_events_query(conn, filter)
}

// The actual query builder, separated so the IN-clause and AND clauses
// compose cleanly without the kind filter's params needing to be re-threaded
// through the optional filters.
fn read_story_events_query(
    conn: &Connection,
    filter: &StoryFilter,
) -> Result<Vec<StoryEvent>> {
    let mut sql = String::from(
        "SELECT seq, event_id, trace_id, ts_iso, ts_nanos, kind, agent, path,
                hash_before, hash_after, detail_json, refs_json
         FROM story_events",
    );
    ...
    if let Some(path) = &filter.path {
        where_clauses.push("path = ?".to_string());
        params.push(Box::new(path.clone()));
    }
    if let Some(trace_id) = &filter.trace_id {
        where_clauses.push("trace_id = ?".to_string());
        params.push(Box::new(trace_id.clone()));
    }
    if let Some(after) = filter.after_seq {
        where_clauses.push("seq > ?".to_string());
    ...
    let rows = stmt.query_map(param_refs.as_slice(), |row| {
        ...
        let refs_text: String = row.get("refs_json")?;
        let detail: serde_json::Value =
            serde_json::from_str(&detail_text).unwrap_or(serde_json::Value::Null);
        let refs: Vec<String> = serde_json::from_str(&refs_text).unwrap_or_default();
        Ok(StoryEvent {
            event_id: row.get("event_id")?,
            trace_id: row.get("trace_id")?,
            ts_iso: row.get("ts_iso")?,
            ts_nanos: row.get::<_, i64>("ts_nanos")? as u64,
            kind: row.get("kind")?,
            agent: row.get("agent")?,
            path: row.get("path")?,
...
/// The seq of the most recently appended event, or 0 if the ledger is empty.
/// Used by the `trelane story` renderer to size progress displays.
pub fn latest_story_seq(conn: &Connection) -> Result<i64> {
    let n: i64 = conn
        .query_row("SELECT COALESCE(MAX(seq), 0) FROM story_events", [], |r| {
            r.get(0)
        })
        .unwrap_or(0);
    Ok(n)
...
/// The "rework detection" query from the spec: groups file_change rows by
/// (path, hash_after) and reports any that appear more than once -- a file
/// reached byte-identical content more than once, i.e. it was changed away
/// and then rebuilt/reverted to the same bytes. The "work redone and
/// overwritten" signature.
///
/// Returns (path, hash_after, times_reached, seqs_csv) per offending group,
/// ordered by times_reached desc.
pub fn detect_rework(conn: &Connection) -> Result<Vec<ReworkHit>> {
    let mut stmt = conn.prepare(
        "SELECT path, hash_after, COUNT(*) AS times_reached,
                GROUP_CONCAT(seq) AS at_seqs
         FROM story_events
    ...
    let rows = stmt.query_map([], |r| {
        Ok(ReworkHit {
            ...
            hash_after: r.get::<_, String>(1)?,
            times_reached: r.get::<_, i64>(2)?,
            at_seqs: r
...
/// The "duplicate-build detection" query from the spec: same content hash
/// appears at multiple distinct paths -- the same file materialized in more
/// than one place (copy/duplicate), the pattern seen in the juice-shop repo.
pub fn detect_duplicate_builds(conn: &Connection) -> Result<Vec<DuplicateBuildHit>> {
    let mut stmt = conn.prepare(
        "SELECT hash_after, COUNT(DISTINCT path) AS n_paths,
    ...
    let rows = stmt.query_map([], |r| {
        Ok(DuplicateBuildHit {
            hash_after: r.get::<_, String>(0)?,
            n_paths: r.get::<_, i64>(1)?,
            paths: r
...
pub struct ReworkHit {
    pub path: String,
    pub hash_after: String,
    pub times_reached: i64,
    pub at_seqs: String,
}

#[derive(Debug, Clone, PartialEq)]
pub struct DuplicateBuildHit {
    pub hash_after: String,
    pub n_paths: i64,
    pub paths: String,
}
...
#[cfg(test)]
mod story_ledger_tests {
    use super::*;
    use crate::db::open_in_memory;
    use crate::models::{Lease, ParkedTask, StoryFilter};

    fn lease(path: &str, holder: &str) -> Lease {
        Lease {
            path: path.to_string(),
    ...
    fn parked(task: &str, agent: &str, wait_path: Option<&str>) -> ParkedTask {
        ParkedTask {
            task: task.to_string(),
            agent: agent.to_string(),
            wait_type: "test".to_string(),
            wait_re: None,
            wait_path: wait_path.map(str::to_string),
            waiting_on: "test_cond".to_string(),
            resume_hint: String::new(),
            created_at: "2026-07-19T00:00:00Z".to_string(),
        }
    }

    // ---- append/read round trip + append-only behavior ----

    #[test]
    fn append_is_insert_only_and_seq_is_monotonic() {
        let conn = open_in_memory().unwrap();
        let ev1 = StoryEvent::new("run_start", Some("alpha".into()))
            .detail(serde_json::json!({"reason": "first"}));
        let ev2 = StoryEvent::new("run_end", Some("alpha".into()))
            .detail(serde_json::json!({"exit": "done"}));
        append_story_event(&conn, &ev1).unwrap();
        append_story_event(&conn, &ev2).unwrap();
        let seqs: Vec<i64> = conn
            .prepare("SELECT seq FROM story_events ORDER BY seq")
            .unwrap()
            .query_map([], |r| r.get(0))
            .unwrap()
            .map(|r| r.unwrap())
            .collect();
        assert_eq!(seqs, vec![1, 2], "autoincrement produces 1, 2");

        // A second append never mutates the first row -- the row's event_id
        // is the same as what we wrote (insert-only, no UPDATE).
        let ev3 = StoryEvent::new("run_start", Some("beta".into()));
        append_story_event(&conn, &ev3).unwrap();
        let first_event_id: String = conn
            .query_row(
                "SELECT event_id FROM story_events WHERE seq = 1",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(first_event_id, ev1.event_id);
    }

    // ---- best-effort: a ledger failure must never fail the caller ----

    #[test]
    fn insert_claim_still_succeeds_when_ledger_fails() {
        // Drop the story_events table so append_story_event fails. insert_claim
        // must still return Ok(true) and the claim must be in the claims
        // table -- the `let _ =` wrapping holds (R16).
        let conn = open_in_memory().unwrap();
        conn.execute_batch("DROP TABLE story_events").unwrap();
        let acquired = insert_claim(&conn, &lease("src/foo.rs", "alpha")).unwrap();
        assert!(acquired, "claim acquired despite ledger being broken");
        let still_held = conn
            .query_row(
                "SELECT COUNT(*) FROM claims WHERE path = 'src/foo.rs'",
                [],
                |r| r.get::<_, i64>(0),
            )
            .unwrap();
    ...
    #[test]
    fn insert_parked_task_still_succeeds_when_ledger_fails() {
        let conn = open_in_memory().unwrap();
        conn.execute_batch("DROP TABLE story_events").unwrap();
        insert_parked_task(&conn, &parked("t1", "alpha", None)).unwrap();
        let n: i64 = conn
    ...
    #[test]
    fn insert_claim_emits_claim_acquired() {
        let conn = open_in_memory().unwrap();
        assert!(insert_claim(&conn, &lease("src/foo.rs", "alpha")).unwrap());
        let evs = read_story_events(&conn, &StoryFilter::default()).unwrap();
        assert_eq!(evs.len(), 1);
        assert_eq!(evs[0].kind, "claim_acquired");
        assert_eq!(evs[0].agent.as_deref(), Some("alpha"));
        assert_eq!(evs[0].path.as_deref(), Some("src/foo.rs"));
    }

    #[test]
    fn insert_claim_emits_claim_denied_when_already_held() {
        let conn = open_in_memory().unwrap();
        assert!(insert_claim(&conn, &lease("src/foo.rs", "alpha")).unwrap());
        let acquired = insert_claim(&conn, &lease("src/foo.rs", "beta")).unwrap();
        assert!(!acquired, "second claim was denied");
        let evs = read_story_events(&conn, &StoryFilter::default()).unwrap();
        assert_eq!(evs.len(), 2);
        assert_eq!(evs[1].kind, "claim_denied");
        assert_eq!(evs[1].agent.as_deref(), Some("beta"));
        assert_eq!(evs[1].path.as_deref(), Some("src/foo.rs"));
        // current_holder is enriched from the claims table.
        let holder = evs[1].detail["current_holder"].as_str();
        assert_eq!(holder, Some("alpha"));
    }

    #[test]
    fn delete_claim_emits_claim_released_explicit() {
        let conn = open_in_memory().unwrap();
        assert!(insert_claim(&conn, &lease("src/foo.rs", "alpha")).unwrap());
        delete_claim(&conn, "src/foo.rs").unwrap();
        let releases = read_story_events(
            &conn,
            &StoryFilter {
                kinds: vec!["claim_released".to_string()],
                ..Default::default()
            },
        )
        .unwrap();
        assert_eq!(releases.len(), 1);
        assert_eq!(releases[0].agent.as_deref(), Some("alpha"));
        assert_eq!(releases[0].detail["via"].as_str(), Some("explicit"));
    }

    #[test]
    fn release_claims_for_delegation_emits_one_per_path() {
        let conn = open_in_memory().unwrap();
        // Insert two claims on the same delegation.
        let mut a = lease("src/a.rs", "alpha");
        a.delegation_id = Some("del-1".to_string());
        let mut b = lease("src/b.rs", "beta");
        b.delegation_id = Some("del-1".to_string());
        assert!(insert_claim(&conn, &a).unwrap());
        assert!(insert_claim(&conn, &b).unwrap());
        // And a third on a different delegation -- should NOT be released.
        let mut c = lease("src/c.rs", "gamma");
        c.delegation_id = Some("del-2".to_string());
        assert!(insert_claim(&conn, &c).unwrap());

        let released = release_claims_for_delegation(&conn, "del-1").unwrap();
        assert_eq!(released, 2);
        let release_events = read_story_events(
            &conn,
            &StoryFilter {
                kinds: vec!["claim_released".to_string()],
                ..Default::default()
        ...
        // Both via "delegation", both with the right delegation_id.
        for ev in &release_events {
            assert_eq!(ev.detail["via"].as_str(), Some("delegation"));
            assert_eq!(ev.detail["delegation_id"].as_str(), Some("del-1"));
        }
        // The third claim on del-2 is still held.
        assert!(
            conn.query_row(
                "SELECT COUNT(*) FROM claims WHERE delegation_id = 'del-2'",
                [],
                |r| r.get::<_, i64>(0),
            )
            .unwrap()
    ...
    #[test]
    fn insert_parked_task_emits_park_event() {
        let conn = open_in_memory().unwrap();
        insert_parked_task(&conn, &parked("t1", "alpha", Some("src/foo.rs"))).unwrap();
        let evs = read_story_events(&conn, &StoryFilter::default()).unwrap();
        assert_eq!(evs.len(), 1);
        assert_eq!(evs[0].kind, "park");
        assert_eq!(evs[0].agent.as_deref(), Some("alpha"));
        assert_eq!(evs[0].path.as_deref(), Some("src/foo.rs"));
        assert_eq!(evs[0].detail["wait_type"].as_str(), Some("test"));
    }

    #[test]
    fn delete_parked_task_emits_unpark_event_with_agent() {
        let conn = open_in_memory().unwrap();
        insert_parked_task(&conn, &parked("t1", "alpha", None)).unwrap();
        delete_parked_task(&conn, "t1").unwrap();
        let unparks = read_story_events(
            &conn,
            &StoryFilter {
                kinds: vec!["unpark".to_string()],
                ..Default::default()
            },
        )
        .unwrap();
        assert_eq!(unparks.len(), 1);
        assert_eq!(unparks[0].agent.as_deref(), Some("alpha"));
        assert_eq!(unparks[0].detail["task"].as_str(), Some("t1"));
    }

    // ---- file_change / rework / duplicate-build detection ----

    fn file_change(conn: &Connection, path: &str, after: Option<&str>, before: Option<&str>) {
        let mut ev = StoryEvent::new("file_change", Some("alpha".into())).path(Some(path.into()));
        ev = ev.hashes(before.map(str::to_string), after.map(str::to_string));
        append_story_event(conn, &ev).unwrap();
    }

    #[test]
    fn detect_rework_flags_byte_identical_reappearance() {
        // Per the spec's acceptance test: append file_change (X, H1),
        // (X, H2), (X, H1) again -> the rework query returns one row for
        // (X, H1) with times_reached=2.
        let conn = open_in_memory().unwrap();
        file_change(&conn, "src/X.rs", Some("H1"), None);
        file_change(&conn, "src/X.rs", Some("H2"), Some("H1"));
        file_change(&conn, "src/X.rs", Some("H1"), Some("H2"));
        let hits = detect_rework(&conn).unwrap();
        assert_eq!(hits.len(), 1);
        assert_eq!(hits[0].path, "src/X.rs");
        assert_eq!(hits[0].hash_after, "H1");
        assert_eq!(hits[0].times_reached, 2);
        // The seqs csv lists the two seqs where H1 was reached.
        assert_eq!(hits[0].at_seqs.split(',').count(), 2);
    }

    #[test]
    fn detect_duplicate_builds_flags_same_hash_at_two_paths() {
        let conn = open_in_memory().unwrap();
        file_change(&conn, "src/data/store.js", Some("H"), None);
        file_change(&conn, "src/db/store.js", Some("H"), None);
        let hits = detect_duplicate_builds(&conn).unwrap();
        assert_eq!(hits.len(), 1);
        assert_eq!(hits[0].hash_after, "H");
        assert_eq!(hits[0].n_paths, 2);
        // The two paths are listed (csv, order is unspecified -- check both).
        let paths: Vec<&str> = hits[0].paths.split(',').collect();
        assert!(paths.contains(&"src/data/store.js"));
        assert!(paths.contains(&"src/db/store.js"));
    }

    // ---- trace_id: nullable, but populated when supplied ----

    #[test]
    fn append_story_event_carries_trace_id_when_supplied() {
        let conn = open_in_memory().unwrap();
        let ev = StoryEvent::new("run_start", Some("alpha".into()))
            .trace(Some("trace-abc".to_string()));
        append_story_event(&conn, &ev).unwrap();
        let read = read_story_events(&conn, &StoryFilter::default()).unwrap();
        assert_eq!(read[0].trace_id.as_deref(), Some("trace-abc"));
    }

    #[test]
    fn append_story_event_with_no_trace_id_is_null() {
        let conn = open_in_memory().unwrap();
        let ev = StoryEvent::new("run_start", Some("alpha".into()));
        assert!(ev.trace_id.is_none());
        append_story_event(&conn, &ev).unwrap();
        let read = read_story_events(&conn, &StoryFilter::default()).unwrap();
        assert!(read[0].trace_id.is_none());
    }

    // ---- read filters ----

    #[test]
    fn read_filters_by_kind_and_agent_and_path() {
        let conn = open_in_memory().unwrap();
        file_change(&conn, "src/a.rs", Some("Ha"), None);
        file_change(&conn, "src/b.rs", Some("Hb"), None);
        append_story_event(
            &conn,
            &StoryEvent::new("run_start", Some("beta".into())),
        )
        .unwrap();

        // Filter by kind only.
        let f = StoryFilter {
            kinds: vec!["file_change".to_string()],
            ..Default::default()
        };
        assert_eq!(read_story_events(&conn, &f).unwrap().len(), 2);
        ...
        assert_eq!(read_story_events(&conn, &f).unwrap().len(), 1);
        ...
        let evs = read_story_events(&conn, &f).unwrap();
        assert_eq!(evs.len(), 1);
        assert_eq!(evs[0].path.as_deref(), Some("src/a.rs"));
    }

    #[test]
    fn read_orders_by_seq_ascending() {
        let conn = open_in_memory().unwrap();
        for i in 0..5 {
            append_story_event(
                &conn,
                &StoryEvent::new("run_start", Some(format!("a{i}"))),
            )
            .unwrap();
        }
        let evs = read_story_events(&conn, &StoryFilter::default()).unwrap();
        // The event_ids are unique but we can assert seq ordering by reading
        // directly.
        let seqs: Vec<i64> = conn
            .prepare("SELECT seq FROM story_events ORDER BY rowid")
            .unwrap()
            .query_map([], |r| r.get(0))
            .unwrap()
            .map(|r| r.unwrap())
